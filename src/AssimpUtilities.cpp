...
void printMatrix4x4(aiMatrix4x4 matrix, unsigned int layer) {
    indent(layer);
...
void printAiSceneInfo(const aiScene* scene, AiScenePrintOption option) {
    ...
    if (scene->HasMeshes()) {
        ...
        for (unsigned int i = 0; i < scene->mNumMeshes; i++) {
            ...
            // for this mesh.
            for (int i = 0; i < currentMesh->GetNumUVChannels(); ++i) {
                if (currentMesh->HasTextureCoords(i)) {
                    cout << "Number of texture coordinates for UV(texture) channel " << i << ":" << currentMesh->mNumVertices << endl;
